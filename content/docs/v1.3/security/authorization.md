...
The central v1.3 checks allow a regular `USER` to:
...
Consult the task guide because some workflows add stricter checks and some read-only pages are available to every signed-in user. For example, the v1.3 Audit Log page is not Admin-gated.
